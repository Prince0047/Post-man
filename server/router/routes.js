...
router.post('/create', authenticate, async (req, res) => {
  try {
    const { to, subject, message, schedule, day, date, month, time, times } =
      req.body;
    ...
    const job = scheduler.scheduleJob(rule, async function () {
      ...
      const output = `                 
                        <p>${message}</p>
                        <br>
                        <p>This was send by Postmail web app made by Sonu Bhadana for flipr hackathon 9.0</p>
                        `;
      ...
      let info = await transporter.sendMail({
        from: '"post mailer" <loganxmen0047@gmail.com>',
        to: to,
        subject: subject,
        text: message,
        html: output,
